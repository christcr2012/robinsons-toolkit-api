...
export function makeEmbedder(config = {}) {
    ...
    if (provider === 'openai')
        return new OpenAIEmbedder(config.model, preferQuality, maxCost);
    if (provider === 'claude' || provider === 'voyage')
        return new VoyageEmbedder(config.model, preferQuality, maxCost);
    if (provider === 'ollama')
        return new OllamaEmbedder(config.model);
    if (provider === 'none')
...
class OpenAIEmbedder {
    ...
    dimensions;
    costPer1M;
    api = process.env.OPENAI_BASE_URL ?? 'https://api.openai.com/v1/embeddings';
    constructor(model, preferQuality = false, maxCost = 0.10) {
...
class VoyageEmbedder {
    ...
    dimensions;
    costPer1M;
    api = process.env.VOYAGE_BASE_URL ?? 'https://api.voyageai.com/v1/embeddings';
    constructor(model, preferQuality = false, maxCost = 0.12) {
        // Smart model selection
        if (model) {
            this.model = model;
        }
    ...
    async embed(texts) {
        ...
        if (!key)
            throw new Error('VOYAGE_API_KEY or ANTHROPIC_API_KEY missing');
        try {
            const res = await fetch(this.api, {
                method: 'POST',
                headers: {
 ...
 */
export function estimateEmbeddingCost(tokens, embedder) {
    if (!embedder)
        return 0;
 ...
 */
export function getRecommendedEmbedder() {
    if (process.env.OPENAI_API_KEY) {
