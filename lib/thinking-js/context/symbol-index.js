 ...
 */
export async function buildSymbolIndex(repoRoot = process.cwd()){
...
function normalizeIndexExt(ext){
  const lower = ext.toLowerCase();
  return INDEX_ALIAS[lower] ?? lower;
}

function extractSymbols(filePath, repoRoot): Symbol[] {
  const symbols: Symbol[] = [];
 ...
 */
export function findSymbolDefinition(symbolName, index: SymbolIndex): Symbol | null {
 ...
 */
export function getFileSymbols(file, index: SymbolIndex): Symbol[] {
 ...
 */
export function findCallers(functionName, index: SymbolIndex, repoRoot = process.cwd()): Array {
  const callers: Array = [];
