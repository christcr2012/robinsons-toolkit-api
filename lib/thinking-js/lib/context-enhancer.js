...
export function withContext(
  toolFn: (input: TInput) => TOutput | Promise,
  defaultQuery?: (input: TInput) => string
) {
  return async (input: TInput, ctx?)=> {
    // Call original tool
    ...
    try {
      ...
      // Format evidence
      const contextEvidence = hits.map((hit) => ({
 ...
 */
export function formatWithContext(output, contextEvidence?){
