/** NEO4J Integration - Pure JavaScript */

async function neo4jFetch(credentials, path, options = {}) {
  const url = path;
  const response = await fetch(url, { ...options });
  if (!response.ok) throw new Error(`API error: ${response.status}`);
  return response.json();
}

async function handleNeo4jQueryExecute(credentials, args) {
  const { cypher, params } = args;
  return await fastAPIClient.neo4jQuery(cypher, params);
}

async function handleNeo4jKnowledgeGraphCreateNode(credentials, args) {
  const { label, properties } = args;
  
  // Build property string
  const propEntries = Object.entries(properties).map(([key, value]) => {
    return `${key}: $${key}`;
  });
  const propString = propEntries.join(', ');
  
  const cypher = `
    CREATE (n:${label} {${propString}})
    RETURN n
  `;
  
  return await fastAPIClient.neo4jQuery(cypher, properties);
}

async function handleNeo4jKnowledgeGraphCreateRelationship(credentials, args) {
  const { from_node, to_node, relationship_type, properties = {} } = args;
  
  // Build match patterns
  const fromLabel = from_node.label;
  const toLabel = to_node.label;
  const fromProps = Object.entries(from_node)
    .filter(([key]) => key !== 'label')
    .map(([key, value]) => `${key}: $from_${key}`)
    .join(', ');
  const toProps = Object.entries(to_node)
    .filter(([key]) => key !== 'label')
    .map(([key, value]) => `${key}: $to_${key}`)
    .join(', ');
  
  // Build relationship properties
  const relProps = Object.entries(properties)
    .map(([key, value]) => `${key}: $rel_${key}`)
    .join(', ');
  const relPropsString = relProps ? `{${relProps}}` : '';
  
  const cypher = `
    MATCH (a:${fromLabel} {${fromProps}})
    MATCH (b:${toLabel} {${toProps}})
    CREATE (a)-[r:${relationship_type} ${relPropsString}]->(b)
    RETURN a, r, b
  `;
  
  // Build parameters
  const params = {};
  Object.entries(from_node).forEach(([key, value]) => {
    if (key !== 'label') params[`from_${key}`] = value;
  });
  Object.entries(to_node).forEach(([key, value]) => {
    if (key !== 'label') params[`to_${key}`] = value;
  });
  Object.entries(properties).forEach(([key, value]) => {
    params[`rel_${key}`] = value;
  });
  
  return await fastAPIClient.neo4jQuery(cypher, params);
}

async function handleNeo4jKnowledgeGraphQuery(credentials, args) {
  const { pattern, where, limit = 10 } = args;
  
  let cypher = `MATCH ${pattern}`;
  if (where) {
    cypher += ` WHERE ${where}`;
  }
  cypher += ` RETURN * LIMIT ${limit}`;
  
  return await fastAPIClient.neo4jQuery(cypher);
}

async function handleNeo4jNodeCreate(credentials, args) {
  const { label, properties } = args;
  
  const propEntries = Object.entries(properties).map(([key]) => `${key}: $${key}`);
  const propString = propEntries.join(', ');
  
  const cypher = `CREATE (n:${label} {${propString}}) RETURN n`;
  return await fastAPIClient.neo4jQuery(cypher, properties);
}

async function handleNeo4jNodeGet(credentials, args) {
  const { node_id } = args;
  const cypher = `MATCH (n) WHERE id(n) = $node_id RETURN n`;
  return await fastAPIClient.neo4jQuery(cypher, { node_id: parseInt(node_id) });
}

async function handleNeo4jNodeUpdate(credentials, args) {
  const { node_id, properties } = args;
  
  const propEntries = Object.entries(properties).map(([key]) => `n.${key} = $${key}`);
  const propString = propEntries.join(', ');
  
  const cypher = `
    MATCH (n) WHERE id(n) = $node_id
    SET ${propString}
    RETURN n
  `;
  
  return await fastAPIClient.neo4jQuery(cypher, { node_id: parseInt(node_id), ...properties });
}

async function handleNeo4jNodeDelete(credentials, args) {
  const { node_id, detach = true } = args;
  const detachKeyword = detach ? 'DETACH ' : '';
  const cypher = `MATCH (n) WHERE id(n) = $node_id ${detachKeyword}DELETE n`;
  return await fastAPIClient.neo4jQuery(cypher, { node_id: parseInt(node_id) });
}

async function handleNeo4jNodeSearch(credentials, args) {
  const { label, properties, limit = 10 } = args;
  
  let cypher = 'MATCH (n';
  if (label) {
    cypher += `:${label}`;
  }
  
  if (properties && Object.keys(properties).length > 0) {
    const propEntries = Object.entries(properties).map(([key]) => `${key}: $${key}`);
    cypher += ` {${propEntries.join(', ')}}`;
  }
  
  cypher += `) RETURN n LIMIT ${limit}`;
  
  return await fastAPIClient.neo4jQuery(cypher, properties || {});
}

async function handleNeo4jRelationshipCreate(credentials, args) {
  const { from_node_id, to_node_id, type, properties = {} } = args;
  
  const propEntries = Object.entries(properties).map(([key]) => `${key}: $${key}`);
  const propString = propEntries.length > 0 ? `{${propEntries.join(', ')}}` : '';
  
  const cypher = `
    MATCH (a) WHERE id(a) = $from_node_id
    MATCH (b) WHERE id(b) = $to_node_id
    CREATE (a)-[r:${type} ${propString}]->(b)
    RETURN r
  `;
  
  return await fastAPIClient.neo4jQuery(cypher, {
    from_node_id: parseInt(from_node_id),
    to_node_id: parseInt(to_node_id),
    ...properties,
  });
}

async function handleNeo4jRelationshipGet(credentials, args) {
  const { relationship_id } = args;
  const cypher = `MATCH ()-[r]->() WHERE id(r) = $relationship_id RETURN r`;
  return await fastAPIClient.neo4jQuery(cypher, { relationship_id: parseInt(relationship_id) });
}

async function handleNeo4jRelationshipDelete(credentials, args) {
  const { relationship_id } = args;
  const cypher = `MATCH ()-[r]->() WHERE id(r) = $relationship_id DELETE r`;
  return await fastAPIClient.neo4jQuery(cypher, { relationship_id: parseInt(relationship_id) });
}

async function handleNeo4jRelationshipSearch(credentials, args) {
  const { type, limit = 10 } = args;
  
  let cypher = 'MATCH ()-[r';
  if (type) {
    cypher += `:${type}`;
  }
  cypher += `]->() RETURN r LIMIT ${limit}`;
  
  return await fastAPIClient.neo4jQuery(cypher);
}

async function handleNeo4jPatternMatch(credentials, args) {
  const { pattern, where, return_clause = '*', limit = 10 } = args;
  
  let cypher = `MATCH ${pattern}`;
  if (where) {
    cypher += ` WHERE ${where}`;
  }
  cypher += ` RETURN ${return_clause} LIMIT ${limit}`;
  
  return await fastAPIClient.neo4jQuery(cypher);
}

async function handleNeo4jPathFind(credentials, args) {
  const { from_node, to_node, max_depth = 5, relationship_types } = args;
  
  // Build node match patterns
  const fromLabel = from_node.label;
  const toLabel = to_node.label;
  const fromProps = Object.entries(from_node)
    .filter(([key]) => key !== 'label')
    .map(([key]) => `${key}: $from_${key}`)
    .join(', ');
  const toProps = Object.entries(to_node)
    .filter(([key]) => key !== 'label')
    .map(([key]) => `${key}: $to_${key}`)
    .join(', ');
  
  // Build relationship type filter
  let relTypeFilter = '';
  if (relationship_types && relationship_types.length > 0) {
    relTypeFilter = `:${relationship_types.join('|')}`;
  }
  
  const cypher = `
    MATCH path = shortestPath(
      (a:${fromLabel} {${fromProps}})-[${relTypeFilter}*1..${max_depth}]-(b:${toLabel} {${toProps}})
    )
    RETURN path
  `;
  
  // Build parameters
  const params = {};
  Object.entries(from_node).forEach(([key, value]) => {
    if (key !== 'label') params[`from_${key}`] = value;
  });
  Object.entries(to_node).forEach(([key, value]) => {
    if (key !== 'label') params[`to_${key}`] = value;
  });
  
  return await fastAPIClient.neo4jQuery(cypher, params);
}

async function handleNeo4jSchemaGet(credentials, args) {
  const cypher = 'CALL db.schema.visualization()';
  return await fastAPIClient.neo4jQuery(cypher);
}

async function handleNeo4jStatsGet(credentials, args) {
  const cypher = `
    MATCH (n)
    WITH labels(n) AS labels, count(*) AS nodeCount
    UNWIND labels AS label
    RETURN label, sum(nodeCount) AS count
    ORDER BY count DESC
  `;
  return await fastAPIClient.neo4jQuery(cypher);
}

async function handleNeo4jConnectionTest(credentials, args) {
  try {
    const result = await fastAPIClient.neo4jGetLabels();
    return {
      success: true,
      message: 'Connection successful',
      labels: result,
    };
  } catch (error) {
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Connection failed',
    };
  }
}

async function handleNeo4jDatabaseList(credentials, args) {
  const cypher = 'SHOW DATABASES';
  return await fastAPIClient.neo4jQuery(cypher);
}

async function handleNeo4jConstraintCreate(credentials, args) {
  const { constraint_name, label, property, type } = args;
  
  let cypher = '';
  switch (type) {
    case 'UNIQUE':
      cypher = `CREATE CONSTRAINT ${constraint_name} IF NOT EXISTS FOR (n:${label}) REQUIRE n.${property} IS UNIQUE`;
      break;
    case 'EXISTS':
      cypher = `CREATE CONSTRAINT ${constraint_name} IF NOT EXISTS FOR (n:${label}) REQUIRE n.${property} IS NOT NULL`;
      break;
    case 'NODE_KEY':
      cypher = `CREATE CONSTRAINT ${constraint_name} IF NOT EXISTS FOR (n:${label}) REQUIRE n.${property} IS NODE KEY`;
      break;
  }
  
  return await fastAPIClient.neo4jQuery(cypher);
}

async function executeNeo4jTool(toolName, args, credentials) {
  const tools = {
    'neo4j_handleNeo4jQueryExecute': handleNeo4jQueryExecute,
    'neo4j_handleNeo4jKnowledgeGraphCreateNode': handleNeo4jKnowledgeGraphCreateNode,
    'neo4j_handleNeo4jKnowledgeGraphCreateRelationship': handleNeo4jKnowledgeGraphCreateRelationship,
    'neo4j_handleNeo4jKnowledgeGraphQuery': handleNeo4jKnowledgeGraphQuery,
    'neo4j_handleNeo4jNodeCreate': handleNeo4jNodeCreate,
    'neo4j_handleNeo4jNodeGet': handleNeo4jNodeGet,
    'neo4j_handleNeo4jNodeUpdate': handleNeo4jNodeUpdate,
    'neo4j_handleNeo4jNodeDelete': handleNeo4jNodeDelete,
    'neo4j_handleNeo4jNodeSearch': handleNeo4jNodeSearch,
    'neo4j_handleNeo4jRelationshipCreate': handleNeo4jRelationshipCreate,
    'neo4j_handleNeo4jRelationshipGet': handleNeo4jRelationshipGet,
    'neo4j_handleNeo4jRelationshipDelete': handleNeo4jRelationshipDelete,
    'neo4j_handleNeo4jRelationshipSearch': handleNeo4jRelationshipSearch,
    'neo4j_handleNeo4jPatternMatch': handleNeo4jPatternMatch,
    'neo4j_handleNeo4jPathFind': handleNeo4jPathFind,
    'neo4j_handleNeo4jSchemaGet': handleNeo4jSchemaGet,
    'neo4j_handleNeo4jStatsGet': handleNeo4jStatsGet,
    'neo4j_handleNeo4jConnectionTest': handleNeo4jConnectionTest,
    'neo4j_handleNeo4jDatabaseList': handleNeo4jDatabaseList,
    'neo4j_handleNeo4jConstraintCreate': handleNeo4jConstraintCreate,
  };
  const handler = tools[toolName];
  if (!handler) throw new Error(`Unknown tool: ${toolName}`);
  return handler(credentials, args);
}

module.exports = { executeNeo4jTool };