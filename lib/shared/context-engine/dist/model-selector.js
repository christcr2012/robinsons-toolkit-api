 ...
 */
function calculateValueScore(qualityScore, costPer1M) {
    if (costPer1M === 0)
        return 100; // Free is infinite value
    return Math.min(100, (qualityScore / costPer1M) * 0.5);
}
/**
 * Score a model for a specific task
 */
function scoreModelForTask(provider, modelName, task) {
 ...
 */
function generateReasoning(provider, model, task, score) {
    const providerModels = MODEL_DATABASE[provider];
    const modelInfo = providerModels[model];
    const reasons = [];
    // Task type match
 ...
 */
export function detectTaskType(fileExtensions) {
 ...
 */
export function estimateComplexity(fileCount, avgFileSize) {
    if (fileCount < 100 && avgFileSize < 1000)
        return 'simple';
    if (fileCount < 1000 && avgFileSize < 5000)
        return 'medium';
    return 'complex';
}
/**
