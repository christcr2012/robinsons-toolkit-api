 ...
 */
export async function buildCodeGraph(root) {
...
export function retrieveContext(
  graph: CodeGraph,
  changedFiles,
  opts: { maxNeighbors?; maxCallSites: number } = {}
): {
  changed;
  neighbors;
  callSites;
 ...
 */
export function findSymbol(graph: CodeGraph, symbolName): {
  definitions;
  references;
} {
  return {
 ...
 */
function inferSymbolKind(name): SymbolDef['kind'] {
  if (/^[A-Z]/.test(name)) {
 ...
 */
function extractUsedSymbols(content) {
  const symbols = new Set();
