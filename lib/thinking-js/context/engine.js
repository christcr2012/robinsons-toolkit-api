...
// Import graph type
  ...
  private patternUpdate?;
  private bootstrapPromise: Promise | null = null;
  private activeIndexPromise: Promise | null = null;
  private backgroundQueue: Array = [];
  private backgroundPromise: Promise | null = null;
  private staleCheckInFlight = false;
  private lastStatsCheck = 0;
  private quickSearch: QuickSearch | null = null;

  private constructor(private root) {
  ...
  async search(query, k= 12){
    ...
    const finalBase = ce
      ? [...boosted].sort((a, b) => (ce.find((x) => x.idx === boosted.indexOf(b))?.score ?? 0) -
                                   (ce.find((x) => x.idx === boosted.indexOf(a))?.score ?? 0))
      : boosted;
   ...
   */
  async quickScan(query, limit= 8): Promise>> {
  ...
  async getImportGraph() {
    ...
    return {
      ...
      getDependencyChain: (file, maxDepth= 10) => {
        const visited = new Set();
        const queue: Array = [{ file, depth: 0 }];
      ...
      getDependents: (file, maxDepth= 10) => {
        const visited = new Set();
        const queue: Array = [{ file, depth: 0 }];
  ...
  private schedulePatternRefresh(){
  ...
  private async executeIndex(opts: Parameters[1] = {}): Promise>> {
  ...
  private async runIndex(opts: Parameters[1] = {}): Promise>> {
  ...
  private async rebuildSymbolIndex(){
  ...
  private enqueueBackgroundIndex(task: { include?; force?; reason?: string } = {}){
  ...
  private async maybeTriggerRefresh(){
    if (this.staleCheckInFlight) return;

    const now = Date.now();
    if (now - this.lastStatsCheck  ttlMs) {
