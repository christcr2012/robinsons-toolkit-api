...
function safeSlug(s) { return slugify(s, { lower: true, strict: true }); }

async function writeArtifact(name, data: Json | string) {
...
function pick(arr: T[], n) { return arr.slice(0, Math.max(0, Math.min(n, arr.length))); }
...
async function webSearch(query, maxResults, provider?) {
  ...
  const results: { url; title?; snippet?: string }[] = [];
  // 1) Tavily
...
async function crawlStep(seeds, opts: { maxPages; sameDomain?; provider?: string }) {
  const seen = new Set();
  const out = [];
...
export function getWebContextTools(): Tool[] {
  return [
    ...
    },
    {
      ...
      handler: async (args) => {
        const seeds = Array.isArray(args.seeds) ? args.seeds : [String(args.seeds)];
