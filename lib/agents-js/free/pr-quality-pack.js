...
/**
 * Render PR Quality Pack
 */
export function renderPRQualityPack(pack: PRQualityPack){
  let md = `# ${pack.title}\n\n`;
 ...
 * Generate test plan
 */
function generateTestPlan(card: DesignCard, exec: ExecReport){
 ...
 */
function generateRollback(card: DesignCard, changedFiles){
 ...
 */
function generateMigrationSteps(card: DesignCard) {
  const steps = [];
  ...
  for (const entity of card.dataModel.entities) {
    ...
    if (entity.removeFields) {
      steps.push(`Mark ${entity.removeFields.join(', ')} in ${entity.name}`);
 ...
 */
function generateDiagrams(card: DesignCard) {
  const diagrams = [];
