 ...
 */
function chunkText(text): { start; end; text: string }[] {
  const lines = text.split(/\r?\n/);
  const out: { start; end; text: string }[] = [];
  let buf = [];
  let start = 1;

  for (let i = 0; i = MAXCH || i === lines.length - 1) {
...
export class FileWatcher {
  ...
  private fileHashes = new Map();
  private isIndexing = false;

  constructor(private repoRoot) {
    this.loadFileHashes();
  }
   ...
   */
  private async hasFileChanged(filePath){
    try {
   ...
   */
  private async reindexFile(filePath){
    if (this.isIndexing) {
   ...
   */
  private handleFileDelete(filePath){
