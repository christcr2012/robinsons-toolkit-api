...
export class OpenAIAdapter implements ModelAdapter {
  ...
  async generateText({ system, input, model = 'gpt-4o-mini' }: GenArgs) {
  ...
  async generateJSON({ system, input, model = 'gpt-4o-mini' }: GenArgs) {
    const res = await this.client.chat.completions.create({
      model,
...
export class AnthropicAdapter implements ModelAdapter {
  ...
  async generateText({ system, input, model = 'claude-3-5-sonnet-latest' }: GenArgs) {
  ...
  async generateJSON({ system, input, model = 'claude-3-5-sonnet-latest' }: GenArgs) {
    const res = await this.client.messages.create({
      model,
      max_tokens: 4096,
      system: system + '\nReturn ONLY strict JSON.',
...
export class OllamaAdapter implements ModelAdapter {
  ...
  async call(model, prompt, format: 'json') {
  ...
  async generateText({ system, input, model = 'qwen2.5-coder:7b' }: GenArgs) {
    const prompt = `${system}\n\nUSER:\n${JSON.stringify(input)}\n\nASSISTANT:`;
    return await this.call(model, prompt);
  }
  async generateJSON({ system, input, model = 'qwen2.5-coder:7b' }: GenArgs) {
    const prompt = `${system}\nReturn ONLY strict JSON.\n\nINPUT:\n${JSON.stringify(input)}`;
