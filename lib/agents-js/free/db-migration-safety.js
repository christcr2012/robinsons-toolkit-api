 ...
 */
export function generateMigrationPlan(card: DesignCard) {
  const phases = [];
  const risks = [];
  const rollbackSteps = [];
  ...
  for (const entity of card.dataModel.entities) {
    ...
    if (entity.addFields && entity.addFields.length > 0) {
      ...
      const expandOps = entity.addFields.map(field => ({
        type: 'add-column',
        table: entity.name,
        column: field.name,
        details: {
      ...
      const contractOps = entity.addFields
        .filter(f => !f.nullable)
        .map(field => ({
          type: 'change-type',
          table: entity.name,
    ...
    if (entity.removeFields && entity.removeFields.length > 0) {
      // Phase 1: EXPAND - Mark (no DB change)
      phases.push({
        name: `Expand: Mark ${entity.name} columns`,
        operations: [],
        canRollback: true,
        requiresDowntime: false,
      });
      ...
      const contractOps = entity.removeFields.map(field => ({
        type: 'remove-column',
        table: entity.name,
        column: field,
        details: {},
 ...
 */
export function checkMigrationSafety(plan: MigrationPlan) {
  const violations = [];
  const warnings = [];
 ...
 */
export function generateMigrationSQL(plan: MigrationPlan, dialect: 'postgres' | 'mysql' = 'postgres') {
  const sqls = [];
 ...
 */
export function renderMigrationPlan(plan: MigrationPlan){
