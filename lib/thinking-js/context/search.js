...
export function lexicalRank(query, text){
...
export async function hybridQuery(query, topK = 8){
  // Check cache first
  ...
  try {
    for (const emb of readJSONL(paths.embeds)) {
  ...
  const scored: Hit[] = [];
  try {
    for (const rawChunk of readJSONL(paths.chunks)) {
