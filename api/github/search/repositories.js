/**
 * GitHub Search Repositories REST API
...
module.exports = async (req, res) => {
  ...
  try {
    if (method === 'GET') {
      ...
      const url = `https://api.github.com/search/repositories?q=${encodeURIComponent(q)}&sort=${sort}&order=${order}&per_page=${per_page}&page=${page}`;
      const response = await fetch(url, { headers });
      if (!response.ok) throw new Error(`GitHub API error: ${response.status}`);
      const data = await response.json();
      return res.status(200).json(checkResponseSize({
        total_count: data.total_count,
        items: data.items?.slice(0, per_page).map(minimalRepo) || []
      }));
    }
