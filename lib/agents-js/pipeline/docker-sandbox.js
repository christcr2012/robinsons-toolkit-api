 ...
 */
async function writeFiles(tempDir, genResult: GenResult) {
 ...
 */
async function copyTemplates(tempDir) {
...
function parseResults(results: {
  format: { stdout; stderr; exitCode: number };
  lint: { stdout; stderr; exitCode: number };
  type: { stdout; stderr; exitCode: number };
  test: { stdout; stderr; exitCode: number };
}) {
  // Parse lint errors
  const lintErrors = [];
  try {
