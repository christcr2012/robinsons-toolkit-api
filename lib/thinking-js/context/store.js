...
export function contextRootPath(){
  return root;
}
...
export function* readJSONL(p): Generator {
...
export function loadDocs(): StoredDoc[] {
  return Array.from(readJSONL(P.docs));
}

// --- File map (path -> {mtimeMs, size, contentSha, chunkIds[]})
export function loadFileMap(): Record {
  try {
...
export function deleteChunksForFile(file) {
  ...
  try {
    ...
    try {
      ...
      for (const chunk of readJSONL(P.chunks)) {
...
function cacheKey(model, sha){
  return `${model}|${sha}`;
}

export function getCachedVec(model, sha) | undefined {
  try {
...
export function putCachedVec(model, sha, vec) {
  const p = path.join(P.embedCache, `${model}.jsonl`);
