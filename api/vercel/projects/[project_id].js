/**
 * Vercel Single Project REST API
 * GET    /api/vercel/projects/[project_id] - Get project
 * PATCH  /api/vercel/projects/[project_id] - Update project
 * DELETE /api/vercel/projects/[project_id] - Delete project
 */
...
module.exports = async (req, res) => {
  ...
  try {
    if (method === 'GET') {
      const url = `${VERCEL_API_BASE}/v9/projects/${project_id}`;
    ...
    if (method === 'PATCH') {
      const { name, framework, buildCommand, outputDirectory } = req.body;
      const url = `${VERCEL_API_BASE}/v9/projects/${project_id}`;
