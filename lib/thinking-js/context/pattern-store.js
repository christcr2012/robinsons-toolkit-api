...
export function savePatternSnapshot(snapshot: PatternSnapshot){
  try {
...
export function analyzePatterns(symbolIndex: SymbolIndex, importGraph: Array = [], chunks: Chunk[] = loadChunks()): PatternSnapshot {
...
export function applyPatternBoosts(query, candidates: Array, snapshot: PatternSnapshot | null): Array {
...
function detectArchitecture(symbolIndex: SymbolIndex, importGraph: Array): ArchitecturalPattern[] {
...
function detectStyle(symbolIndex: SymbolIndex, chunks: Chunk[]): StylePattern[] {
  ...
  const namingConfidence = (count) => Math.min(1, count / totalNames);
