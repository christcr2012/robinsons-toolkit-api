/*
  ...
  Mark tests if non-deterministic.
...
export async function detectFlakyTests(
  ...
  for (const test of failedTests) {
    ...
    for (let i = 0; i  r.passed).length;
    const passRate = passed / runs.length;
    
    // Determine if flaky
    const isFlaky = passRate > 0 && passRate  r.recommendation === 'quarantine')
    .map(r => r.test);
 ...
 */
export function renderFlakyTestReport(reports){
 ...
 */
export function suggestFlakyTestFixes(report: FlakyTestReport) {
  const suggestions = [];
