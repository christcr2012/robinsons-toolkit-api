...
export async function docsAuditTool(
  args: { repoPath?; planTypes?; completionTypes?; k?: number },
  ctx) {
  ...
  try {
    ...
    const isType = (d, set) => set.includes(String(d.type||"other").toLowerCase());

    const plans = docs.filter((d) => isType(d, planTypes));
    const finals = docs.filter((d) => isType(d, completionTypes));

    const results= { repoRoot: path.resolve(repoPath), plans:[], finals:[], missing:[], stale:[], conflicts:[] };
