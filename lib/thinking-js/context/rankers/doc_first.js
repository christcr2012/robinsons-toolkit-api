...
export function scoreDoc(q, d: StoredDoc) {
  const s = q.toLowerCase();
  let score = 0;

  // term matches (title then summary)
  const hit = (t) => (t.toLowerCase().includes(s) ? 1 : 0);
  ...
  // status priors
  const statusW: Record = { approved: 0.8, done: 1.0, 'in-progress': 0.4, draft: 0.2, deprecated: -0.8, unknown: 0 };
...
export function rerankDocs(q, docs: StoredDoc[], k = 24){
