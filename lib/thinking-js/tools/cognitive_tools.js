...
async function write(base, json: J, md) {
  await ensureDir(OUT);
...
function pick(lines, needles: RegExp[], max = 8) {
...
async function readEvidence(paths) {
  const out: { path; text: string }[] = [];
...
const think_decision_matrix: ToolDef = {
  ...
  handler: async ({ title, options, criteria, weights }) => {
    ...
    const criteriaEntries = criteria.map((name, index) => ({
      name,
    ...
    const optionRows = (options[]).map((optionName) => ({
      option: optionName,
