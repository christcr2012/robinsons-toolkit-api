...
import { ollamaGenerate, pingOllama } from '@robinson_ai_systems/shared-llm';
...
export class OllamaClient {
   ...
   * Get model info
   */
  getModelInfo(modelName): ModelConfig | undefined {
  ...
  async startOllama() {
    ...
    try {
      ...
      while (totalWait  setTimeout(resolve, delay));
        totalWait += delay;
        attemptCount++;
  ...
  async ensureRunning() {
    ...
    try {
      ...
      if (this.autoStart && !error.message?.includes('auto-start') && !error.message?.includes('started but still not responding')) {
        console.error('[OllamaClient] Ping failed, trying auto-start...');
        await this.startOllama();
      } else {
        throw error;
      }
