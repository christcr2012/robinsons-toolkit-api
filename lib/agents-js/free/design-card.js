 ...
 */
export function parseDesignCard(filePath): DesignCard {
...
function parseSimpleYAML(content){
  ...
  const result= {};
  const stack: Array = [{ obj: result, indent: -1 }];
  ...
  for (let line of lines) {
    ...
    while (stack.length > 1 && indent  s.trim());
 ...
 */
export function validateDesignCard(card): DesignCard {
  const errors = [];
 ...
 */
export function designCardToTaskSpec(card: DesignCard, brief){
  let spec = `# ${card.name}\n\n`;
 ...
 */
export function findDesignCard(slug, root = process.cwd()){
  const candidates = [
