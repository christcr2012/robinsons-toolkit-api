 ...
 */
async function writeFiles(sandboxDir, genResult: GenResult){
 ...
 */
async function initializeProject(sandboxDir){
 ...
 */
async function runFormatter(sandboxDir){
  try {
 ...
 * Run eslint
 */
async function runLinter(sandboxDir){
  try {
    ...
    const errors = results
      ...
      .map((m) => `${m.ruleId}: ${m.message} (${m.line}:${m.column})`);
 ...
 */
async function runTypeChecker(sandboxDir){
  try {
...
async function runTests(
  ...
  try {
    ...
    return {
      ...
      details: results.testResults?.map((t) => t.message) || [],
 ...
 */
function collectLogs(sandboxDir) {
  const logs = [];
