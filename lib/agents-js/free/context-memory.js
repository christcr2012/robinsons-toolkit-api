...
export class ContextMemory {
   ...
   */
  explainSimilarity(card1: DesignCard, card2: DesignCard){
    const reasons = [];
 ...
 */
export function formatMemoryMatches(matches){
  if (matches.length === 0) {
    return 'No similar past tasks found.';
  }

  let formatted = `## Similar Past Tasks (${matches.length})\n\n`;
  formatted += `These tasks were successfully completed in the past. Use them:\n\n`;
