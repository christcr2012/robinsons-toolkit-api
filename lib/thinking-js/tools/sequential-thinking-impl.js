 ...
 */
export async function sequentialThinkingTool(args, ctx){
  ...
  if (args.thought && args.thoughtNumber !== undefined) {
    const history = ctx.stateGet('seqThinking_history') ?? [];
    const evidence = ctx.stateGet('seqThinking_evidence') ?? [];
    const problem = ctx.stateGet('seqThinking_problem') || 'Analysis';
    ...
    // Get branches if any
    const branches = ctx.stateGet('seqThinking_branches') ?? [];
 ...
 */
export async function parallelThinkingTool(args, ctx){
  const branches = ctx.stateUpdate('seqThinking_branches', (current) => {
 ...
 */
export async function reflectiveThinkingTool(args, ctx){
  const reflections = ctx.stateUpdate('seqThinking_reflections', (current) => {
    const existing = current ?? [];
    existing.push({
      thoughtNumber: args.thoughtNumber,
