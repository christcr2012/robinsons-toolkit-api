...
function upcaseExported(name){
  return /^[A-Z]/.test(name);
}

function notPrefixedUnderscore(name){
 ...
 */
export function extractSymbolMatches(source, fileExt): SymbolMatch[] {
  const ext = (fileExt || '').toLowerCase();
  const lines = source.split(/\r?\n/);
  const results: SymbolMatch[] = [];
  const seen = new Set();

  function add(name, kind: SymbolKind, exported) {
    const key = `${kind}:${name}`;
